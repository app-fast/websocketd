...
func createEnv(handler *WebsocketdHandler, req *http.Request, log *LogScope) []string {
	...
	env = appendEnv(env, "REMOTE_ADDR", handler.Addr)
	env = appendEnv(env, "REMOTE_HOST", handler.Host)
	...
	env = appendEnv(env, "SCRIPT_NAME", handler.ScriptPath)
	env = appendEnv(env, "PATH_INFO", handler.PathInfo)
	...
	env = appendEnv(env, "REMOTE_PORT", handler.Port)
